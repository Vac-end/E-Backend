...
export const userService = {
  ...
  create: async (data: { username: string; email: string; password: string; role: 'niño' | 'docente' | 'administrador'; name?: string; gradeId?: number}) => {
    const hashedPassword = await bcrypt.hash(data.password, 10);
    return userRepository.create({ ...data, password: hashedPassword });
  },
  update: (id: number, data: Partial<{ username: string; email: string; name?: string; gradeId?: number; password?: string }>) => {
